 ...
 *
 *
 * 8-14-2020  532nm laser uses inverted TTL: HIGH = off, LOW = on.  Toggle (Invert) Pin 11 
 * First Set Pin 11 HIGH.  Then toggle the bit 3 from Serial.read command 
 * 
 */
 ...
 void setup() {
  ...
  //Set Pin 11 HIGH to keep 532nm laser off 
     ...
     switch (inByte) {
       ...
       case 1 :
          if (waitForSerial(timeOut_)) {
            ...
            //Use bitwise XOR as toggle for Pin11 (bit 3)
            currentPattern_ = currentPattern_ ^ B00001000;
            if (!blanking_)
          ...
          if (waitForSerial(timeOut_)) {
            ...
            if ( (patternNumber >= 0) && (patternNumber < SEQUENCELENGTH) ) {
              if (waitForSerial(timeOut_)) {
                ...
                //Use bitwise XOR as toggle for Pin7 (bit 3)
                triggerPattern_[patternNumber] = triggerPattern_[patternNumber] ^ B00001000;
                Serial.write( byte(5));
